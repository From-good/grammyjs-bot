...
const adminReplyMiddleware = async (ctx, next) => {
    ...
    // Ищем ID в тексте или подписи сообщения, на которое отвечают
...
// 🔥 ИСПРАВЛЕНИЕ: Применяем middleware ко всем типам сообщений, которые может отправить администратор
bot.on([
    'message:text',
    'message:photo',
    'message:document',
